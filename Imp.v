...
(** Zhaoguo Wang **)
(** 1, 2, & 3 star exercises are completed**)
...
Proof.
  intros i1 i2. destruct i1 ; destruct i2.
  intros H. apply beq_nat_eq in H. 
...
Proof.
  apply E_Seq 
    with (update (update empty_state 
    X 2) Y 0).
    apply E_Ass. reflexivity.

  apply E_WhileLoop 
    with (update (update (update (update empty_state 
    X 2) Y 0) Y 2) X 1).
    reflexivity.

  apply E_Seq 
    with (update (update (update empty_state 
    X 2) Y 0) Y 2).
    apply E_Ass. reflexivity.
    apply E_Ass. reflexivity.

  apply E_WhileLoop 
    with (update (update (update (update (update (update empty_state 
    X 2) Y 0) Y 2) X 1) Y 3) X 0).
    reflexivity.

  apply E_Seq
    with (update (update (update (update (update empty_state 
    X 2) Y 0) Y 2) X 1) Y 3).
    apply E_Ass. reflexivity.
    apply E_Ass. reflexivity.
...
Theorem XtimesYinZ_spec : forall st st' nx ny,  st X = nx -> st Y = ny -> XtimesYinZ / st || st' -> st' Z = nx * ny.
Proof.
  intros st st' nx ny Hx Hy Heval.
...
Proof.
  ...
  ceval_cases (induction contra) Case ; try (inversion Heqloopdef ; subst).
  inversion H ; subst.
...
Inductive no_whilesR: com -> Prop :=
  ...
  | nw_assign : forall v e, no_whilesR (v ::= e)
...
Tactic Notation "no_whileR_cases" tactic(first) ident(c) :=
   first ; [Case_aux c "nw_skip" | Case_aux c "nw_assign" |
...
Proof.
   ...
   Case "->".
     ...
     remember (no_whiles c1) as Hc1.
     destruct Hc1 ; apply IHc1 ; try reflexivity.
     inversion H. rewrite  <- HeqHc1. reflexivity.
     remember (no_whiles c2) as Hc2.
     destruct Hc2 ; apply IHc2 ; try reflexivity.
     inversion H. rewrite <- HeqHc2. rewrite -> andb_comm. 
     reflexivity.
     inversion H. apply IHc1. apply andb_true_iff in H1.
     inversion H1 ; assumption.
     inversion H. apply IHc2. apply andb_true_iff in H1.
     inversion H1 ; assumption.
     inversion H.
   Case "<-".
     intros H.
     no_whileR_cases (induction H) SCase ; try reflexivity.
     simpl.
     ...
     match prog with
        ...
        | (i :: is) => match i with
                         ...
                         | SPlus     => match stack with
                                           | [] => stack
                                           | [x] => stack
                                           | (j :: l :: ls) => s_execute st ((j + l) :: ls) is
                                        end
                         | SMinus    => match stack with
                                           | [] => stack
                                           | [x] => stack
                                           | (j :: l :: ls) => s_execute st ((l - j) :: ls) is
                                        end
...
Theorem beval_sc_correct : forall st e,
  beval_sc st e = beval st e.
Proof.
  intros st e.
