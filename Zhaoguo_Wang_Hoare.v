...
(* 1. Define a assertion that a function is one state implies state x =3 
   2. Define a assertion that a function is one state implies another state 
of x <= state y
   3. Define a assertion that a function is one state implies anther two state 
which are x =3 and x <=y
   4. Define a assertion that a function is one state implies 
st Z * st Z <= st X /\~ (((S (st Z)) * (S (st Z))) <= st X).
   5. Define a assertion that a function is one state implies true
   6. Define a assertion that a function is one state implies false *)
...
(**1. if command c is started at precondition Ture,then it will satisfy the postcondition 
x = 5.
   2. if command c is started at precondition X = m,then it will satisfy the postcondition 
X = m + 5. 
   3. if command c is started at precondition X <= Y,then it will satisfy the postcondition 
Y <= X.
   4. if command c is started at precondition Ture,then it will satisfy the postcondition false.
   5. if command c is started at precondition X = m ,then it will satisfy the postcondition 
Y = real_fact m.
   6. if command c is started at precondition True,then it will satisfy the postcondition 
(Z * Z) <= m /\ ~ (((S Z) * (S Z)) <= m).
  *)
...
(* the expression a could contain X, and X could have a 
different value before and after the computation *)
(** [] *)
...
Proof.
  apply hoare_if.
  Case "X<Y".
  eapply hoare_consequence_pre. apply hoare_asgn. unfold bassn.
  unfold assn_sub. unfold assert_implies. simpl. intros. inversion H.
  unfold update. simpl. apply ble_nat_true in H1. omega.
  Case "X>=Y".
  eapply hoare_consequence_pre. apply hoare_asgn.
...
Proof.
  ...
  ceval_cases (induction H0) Case; inversion Heqc0; subst; clear Heqc0; split; auto;
  try (apply IHceval2; auto; apply (H st); auto);
  try (apply (H st); auto).
Qed.
