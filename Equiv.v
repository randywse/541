...
Proof. 
  ...
  split; intros H.
    ...
    assert (bequiv BTrue BTrue) as Hb'.
      split; reflexivity.
    apply WHILE_true_nonterm with (c:=SKIP) (st:=st) (st':=st') in Hb'.
    contradiction.
Qed.
...
Proof.
  intros i st a ni H.
  induction a; inversion H; simpl; auto.
  apply update_neq.
...
Proof.
  unfold not, cequiv.
  intro H.
  remember (H empty_state empty_state).
  inversion i.
...
Proof. 
  left.
  unfold not, cequiv, pXY, pYX.
  intros st st'. split.
  intro H.
  inversion H; subst; clear H.
  inversion H2; subst; clear H2.
  inversion H5; subst; clear H5.
  rename n into nx. rename n0 into ny.
  assert (update (update st X nx) Y ny = update (update st Y ny) X nx).
    apply functional_extensionality. 
    intro x. apply update_permute; reflexivity.
  rewrite H.
  apply E_Seq with (update st Y ny); apply E_Havoc.
  intro H.
  inversion H; subst; clear H.
  inversion H2; subst; clear H2.
  inversion H5; subst; clear H5.
  rename n into ny. rename n0 into nx.
  assert (update (update st Y ny) X nx = update (update st X nx) Y ny).
    apply functional_extensionality.
    intro x. apply update_permute; reflexivity.
  rewrite H.
  apply E_Seq with (update st X nx); apply E_Havoc.
Qed.
...
Proof.
  ...
  intros.
  remember (update (update empty_state X 1) Y 2) as st12.
  remember (H empty_state st12) as Ha.
  inversion Ha.
  assert ((HAVOC X; HAVOC Y) / empty_state || st12) as Hst12.
    subst st12.
    apply E_Seq with (update empty_state X 1).
    apply E_Havoc.
    apply E_Havoc.
  remember (H0 Hst12) as Hst0.
  inversion Hst0. subst.
  inversion H4. subst. clear H4.
  inversion H7. subst. clear H7.
  simpl in H6. rewrite update_eq in H6.
  clear H. clear H0. clear H1. clear Hst12.
