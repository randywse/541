...
Fixpoint factorial (n:nat) : nat := 
   match n with
   | O => 1
   | S n' => plus (factorial n') (mult n' (factorial n'))
   end.
...
(** **** Exercise #5: 1 star, optional (simpl_plus) *)
...
(** Explain the difference.  [simpl does not reduce  0+n to n in the first
one while it does in the second one; The simple tactic simpl first applies
 βι-reduction rule. Then it expands transparent constants and tries to reduce 
T’ according, once more, to βι rules. But when the ι rule is not applicable 
then possible δ-reductions are not applied.   ] *)
...
(** **** Exercise #6: 1 star (plus_id_exercise) *)
...
(** **** Exercise #7: 2 stars (mult_1_plus) *)
...
(** **** Exercise #8: 1 star (zero_nbeq_plus_1) *)
...
(** **** Exercise #9: 2 stars (boolean functions) *)
...
(** **** Exercise #10: 2 stars (andb_eq_orb) *)
(** Prove the following theorem.  (You may need to first prove a
    subsidiary lemma or two.) *)


Lemma lemorb_1: orb true false = true.
  Proof. reflexivity. Qed.
Lemma lemorb_2 : orb false true = true.
  Proof. reflexivity. Qed.
Theorem andb_eq_orb : 
  forall (b c : bool),
  (andb b c = orb b c) ->
...
Proof.
  ...
  destruct b, c.
  reflexivity.
  rewrite <- lemorb_1. rewrite <- H. reflexivity.
  rewrite <- lemorb_2. rewrite <- H. reflexivity.
  reflexivity.
  Qed.
...
(** **** Exercise #11: 2 stars, optional (decreasing) *)
...
(**
Fixpoint divide' (n : nat) (m : nat) : nat :=
  match n with 
  | O => n
  | S m' => S (divide' n m')
  end.
